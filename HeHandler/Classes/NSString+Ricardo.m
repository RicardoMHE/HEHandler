...
#define IV  @"123412341234"
#define  KEY  @"com.55hd.loan"

#import "NSString+Ricardo.h"

#import "NSString+Ricardo.h"
#import "NSData+Ricardo.h"
#import <CommonCrypto/CommonCrypto.h>

@implementation NSString (Ricardo)

- (CGFloat)widthWithFont:(UIFont *)font {
    return [self boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : font} context:nil].size.width;
}
...
- (NSString *)addSpaceWithSpaceBetween:(NSInteger)spaceBetween {
    ...
    for (NSInteger i = 0; i < (NSInteger)(self.length / spaceBetween) + 1; i++) {
        if (i == 0) continue;
        [str insertString:@" " atIndex:spaceBetween * i + (i - 1)];
    }
...
- (NSMutableAttributedString *)he_creatAttWithString:(NSString *)string color:(UIColor *)color {
    ...
    [att addAttribute:NSForegroundColorAttributeName value:color range:[self rangeOfString:string]];
    return att;
}
...
- (BOOL)isBlank {
    if ([self isEqualToString:@"(null)"] || [self isEqualToString:@"<null>"]) {
        return YES;
    } else if ([self rangeOfString:@"null"].location != NSNotFound) {
        return YES;
    } else if (self == nil || self == NULL) {
        return YES;
...
#pragma encryption
#pragma mark - 散列函数
- (NSString *)aesEncryptStrig {
    return [NSString AES128EncryptStrig:self];
}

- (NSString *)aesDecryptString {
    return [NSString AES128DecryptString:self];
}
